<?php
/*
 * from https://github.com/bezumkin/Office/blob/master/core/components/office/processors/profile/update.class.php
 */

require MODX_CORE_PATH.'model/modx/processors/security/user/update.class.php';
class userProfileUserUpdateProcessor extends modUserUpdateProcessor {
		...
		elseif (!empty($_FILES['newphoto']) && preg_match('/image/', $_FILES['newphoto']['type']) && $_FILES['newphoto']['error'] == 0) {
			...
			if ($phpThumb->GenerateThumbnail()) {
				if ($phpThumb->renderToFile($dst)) {
					...
					$this->modx->log(modX::LOG_LEVEL_ERROR, '[UserProfile] Could not save rendered image to "'.$dst.'"');
				}
			}
			else {
				$this->modx->log(modX::LOG_LEVEL_ERROR, '[UserProfile] ' . print_r($phpThumb->debugmessages, true));
			}
		}
		return true;
	}
...
return 'userProfileUserUpdateProcessor';